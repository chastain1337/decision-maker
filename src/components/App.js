...
class App extends React.Component {
    ...
    updateFactor = (factor, index, textBox) => {
        var fact
        if (factor === 'i') {fact = "ind"}
        if (factor === 'd') {fact = "dep"}
        
        this.setState( prevState => {
            var _persons = prevState.persons;
            _persons[index][fact] = textBox.value;
            return{persons: _persons};
        });
    };
    
    componentDidUpdate() {
        // figure out what needs to happen with the calculate button

        if (this.state.calculateButtonDisabled === true) {   // calculate button is disabled
            if (this.allFactorsAreValid()) {    // we need to enable the calc button
        ...
        } else {// the calculate button is enabled
            if (!this.allFactorsAreValid()) {   // if at least one factor is invalid, disable the calc button
    ...
    allFactorsAreValid = () => {
        const found = this.state.persons.find( 
            person => (person.ind === '' || person.dep === '' || isNaN(person.ind) || isNaN(person.dep))
        )
    ...
    updateDecision = () => {
        this.setState( prevState => {
            ...
            return {currentDecision: score + decMessage, score: score, decisionStyle: decisionStyle}
        });
    }
    ...
    addPersonFn = playerName => {
        this.setState( prevState => {
            ...
            _persons.push({name: playerName, ind: '', dep: '', key: newKey.toString()})
            return {persons: _persons};
        })
    }
    ...
    render() {
        return(
            <Container className="border border-primary rounded-lg bg-blue_primary2 p-3">
                <Header formula={this.state.currentFormula} currentDecision={this.state.currentDecision} decisionStyle={this.state.decisionStyle}/>
                <PersonList persons={this.state.persons} updateFactor={this.updateFactor} removePerson={this.removePerson} setCalculateButton={this.setCalculateButton}/>
                <Calculate numPeople={this.state.persons.length} calculateFn={this.calculate} calculateButtonDisabled={this.state.calculateButtonDisabled}/>
